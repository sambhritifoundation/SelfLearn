...
assert(subject,'SCI8 subject missing');assert.equal(subject.chapters.length,19);assert.equal(subject.chapters.reduce((n,c)=>n+c.topics.length,0),76);assert.equal(qs.length,608);
assert.equal(subject.chapters.filter(c=>c.alignment.ncert!=='—').length,13);assert.equal(subject.chapters.filter(c=>c.alignment.jac!=='—').length,15);
...
assert.deepEqual(counts,{mcq:228,multi:76,fill:152,match:76,subjective:76});
for(const lang of ['en','hi']){sandbox.LANG=lang;const intro=sandbox.vSubjectIntro('SCI8'),catalogue=sandbox.vSubject('SCI8');assert(intro.includes('m8-alignment'));assert.equal((intro.match(/<tr>/g)||[]).length,20);assert(catalogue.includes('chapter-index-grid'));assert.equal((catalogue.match(/id="sci8-chapter-/g)||[]).length,19);assert(catalogue.includes('13 NCERT chapters + 5 JAC bridge units + 1 story pilot'));}
for(const c of subject.chapters){assert.equal(c.topics.length,4);assert.equal(c.assessment.length,12);assert.equal(c.revision.length,c.no===19?4:8);assert(c.no===19||c.summary.en.includes('What comes next'));assert(c.no===19||c.summary.hi.includes('आगे क्या आएगा'));
  for(const tp of c.topics){const tq=qs.filter(q=>q.topic===tp.code);assert.equal(tq.length,8,tp.code);for(const lang of ['en','hi']){sandbox.LANG=lang;assert(tp.notes[lang].length>650,tp.code+' short notes '+lang);const rendered=sandbox.vTopic(tp.code);assert(!rendered.includes('{{diagram:'),tp.code);assert(rendered.includes('<svg'),tp.code);if(c.no===19){assert(rendered.includes('SL8.frictionScene'),tp.code);assert(rendered.includes('Story mission'),tp.code);}else{assert(rendered.includes('SL8.exploreScience'),tp.code);assert(rendered.includes('Three examples around you')||rendered.includes('अपने आसपास के तीन examples'),tp.code);assert(rendered.includes('What this prepares you for')||rendered.includes('आगे किस काम आएगा'),tp.code);}assert(!/undefined|NaN/.test(rendered),tp.code+' invalid render');}}
}
// All 72 visuals must be tied to their own topic labels in both languages.
for(const lang of ['en','hi']){sandbox.LANG=lang;const visuals=[];for(const c of subject.chapters.slice(0,18))for(const tp of c.topics){const h=sandbox.DIAGRAMS['s8-'+tp.code.toLowerCase()]();for(const label of tp.lab.labels)assert(h.includes(sandbox.t(label).length>22?sandbox.esc(sandbox.t(label).slice(0,21)) : sandbox.esc(sandbox.t(label))),tp.code+' missing label');visuals.push(h);}assert.equal(new Set(visuals).size,72,'reused science visuals '+lang);}
...
console.log('PASS: 18 aligned science units plus Friction story pilot, 76 bilingual micro-topics, 72 core diagrams, 608 five-format questions, chapter transitions, rich scoring, and spreadsheet round trips.');
