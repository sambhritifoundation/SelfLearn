...
// Each generated explorer must now be tied to its micro-topic, rather than reusing one chapter-level picture.
for(const c of subject.chapters.slice(1,14))for(const lang of ['en','hi']){sandbox.LANG=lang;const labs=c.topics.map(tp=>{const rendered=sandbox.vTopic(tp.code),start=rendered.indexOf('<div class="m8-output"');assert(start>=0,tp.code+' missing explorer output');const end=rendered.indexOf('</section>',start);const lab=rendered.slice(start,end);assert(lab.includes('<svg'),tp.code+' missing SVG');assert(!/undefined|NaN/.test(lab),tp.code+' invalid output');return lab;});assert.equal(new Set(labs).size,4,'reused illustration in chapter '+c.no+' '+lang);}
sandbox.LANG='en';
const visualChecks={'M8-2-3':'2^−3 = 1/2^3','M8-3-1':'groups of five','M8-4-2':'= 360°','M8-5-2':'digit sum','M8-6-3':'− 2 both sides','M8-7-4':'shared in 2:3','M8-8-3':'10% discount','M8-9-2':'each half','M8-10-3':'135°','M8-11-2':'face 1 of 6','M8-12-2':'Dot plot values','M8-13-3':'top sum','M8-14-4':'large rectangle − cut-out'};
for(const [code,phrase] of Object.entries(visualChecks))assert(sandbox.vTopic(code).includes(phrase),code+' illustration mismatch');
