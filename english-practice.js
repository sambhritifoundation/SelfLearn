...
window.EnglishPractice = (() => {
  ...
  function speakTurn() {
    ...
    chatLine('Akshata',turn.system,'system');
    document.getElementById('ep-model-text').textContent=turn.model;document.getElementById('ep-model').open=false;
    status.textContent=tr('Akshata is speaking. Your recording will begin when she finishes.','अक्षता बोल रही है। उसके बाद रिकॉर्डिंग शुरू होगी।');
    if(!('speechSynthesis' in window)||!window.SpeechSynthesisUtterance){status.textContent=tr('Audio is unavailable. Read Akshata’s line; recording will start now.','आवाज़ उपलब्ध नहीं। अक्षता की पंक्ति पढ़ें; रिकॉर्डिंग अब शुरू होगी।');startRoleplayRecording();return;}
  ...
  function checkTurn() {
    ...
    if(roleplay.turn>=lessons[0].conversation.length){save('speaking',0,{done:true,conversationComplete:true});roleplay.active=false;setChatControls(false);document.getElementById('ep-chat-status').textContent=tr('Conversation complete — '+lessons[0].conversation.length+' turns finished. You can play it again whenever you like.','बातचीत पूरी — '+lessons[0].conversation.length+' जवाब हो गए। चाहें तो फिर चलाएँ।');return;}
    document.getElementById('ep-chat-status').textContent=tr('Good. Akshata will continue…','अच्छा। अक्षता आगे बोलेगी…');setTimeout(()=>{if(roleplay.active)speakTurn();},700);
  }
